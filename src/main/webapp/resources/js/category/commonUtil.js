...
//------------------------------------------------------------------------- 
//에디터 이미지 업로드 callback
//-------------------------------------------------------------------------
	function uploadComplete(d){

        //2017.11.1 파일경로 수정
		if(d.result != null && d.result != ""){
			if(d.result == "err_size"){
				alert("용량은 10MB를 초과할 수 없습니다.");
				return;
			}
			if(d.result == "err_ext"){
				alert("이미지(JPG,PNG,GIF,BMP) 파일이 아닙니다.");
				return;
			}
			top.$('.mce-btn.mce-open').parent().find('.mce-textbox').val('/uploads/editorImages/'+ d.editorFormNowYear + "/" + d.result).closest('.mce-window').find('.mce-primary').click();
		}
	}
